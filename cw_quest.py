...
copywriter = Agent(
    ...
    goal="Craft compelling QUEST (Qualify-Understand-Educate-Stimulate-Transition) copy using the given inputs and insights from the researcher.",
...
def generate_quest(Creativity, Product_Name, Audience, Product_Description, Tone_of_Voice):
    # Define the Tasks
    research_task = Task(
    ...
    quest_task = Task(
        description=f"Generate a persuasive marketing copy using the QUEST (Qualify-Understand-Educate-Stimulate-Transition) framework. \n"
                    f"Use the following product insights: {Product_Description}, creativity level: {Creativity}, and tone of voice: {Tone_of_Voice}",
        expected_output="A well-structured marketing copy using the QUEST framework.",
        agent=copywriter,
    ...
    edit_task = Task(
        ...
        agent=editor,
        depends_on=[quest_task]
    )

    parse_task = Task(
        description="Parse and correct the final QUEST marketing copy for any errors or inconsistencies.",
        expected_output="A final, error-free QUEST marketing copy.",
        agent=editor,
    ...
    crew = Crew(
        ...
        tasks=[research_task, quest_task, edit_task, parse_task],
        verbose=True
    )
